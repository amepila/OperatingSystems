/**
  \archivo      filesapi.c  
  \descripcion  Archivo fuente de las apis del 
  				sistema de archivos
  \fecha        17/May/20
 */
...
int vdread(int fd, char *buffer, int bytes)
{
	...
	current_inode = openfiles[fd].inode;
	...
	while(counter < bytes)
	{
		...
		//Lee el bloque al buffer del archivo
		if(openfiles[fd].currbloqueenmemoria != current_block)
		{
			readblock(current_block,openfiles[fd].buffer);
			openfiles[fd].currbloqueenmemoria = current_block;
		}
		...
		//Si la posicion es mayor que el tamano, llegamos al final
		if(openfiles[fd].currpos>=inode[current_inode].size)
			break;
